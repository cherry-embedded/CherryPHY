...
void rtl8211_phy_init(uint8_t busid, uint16_t phy_addr, struct chry_phy_config *config)
{
}

void rtl8211_phy_get_status(uint8_t busid, uint16_t phy_addr, struct chry_phy_status *status)
{
    uint16_t regval;

    regval = chry_mdio_read(busid, phy_addr, RTL8211_PHYSR);
