...
void dp83848_phy_init(uint8_t busid, uint16_t phy_addr, struct chry_phy_config *config)
{
}

void dp83848_phy_get_status(uint8_t busid, uint16_t phy_addr, struct chry_phy_status *status)
{
    uint16_t regval;

    regval = chry_mdio_read(busid, phy_addr, DP83848_PHYSTS);
